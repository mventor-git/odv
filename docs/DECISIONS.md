...
2026-09-14 (014) - Identity enforcement boundary: API guard-rails + resolver + typed audit NOW; legacy session hard-gate only with approved migration (records carry no project context; dual-write blocked until human approval). Consequence: legacy tier auth stays interim until role_assignments map records to projects.
2026-09-14 (013-014) - Enforcement style fixed: data-driven rules + derived windows (never status rewrites for expiry), revoke=flip-never-delete, audit typed person_id when linked. Alternatives: hard-coded per-type machines, soft-delete histories - rejected.
2026-09-14 (015) - Material chain enforcement = facts only: movement history append-only at DB level (triggers mirror audit_events), lot stage/remaining derived from events (lot.status kept for compatibility only — single source of truth), physical balance (a lot cannot give out more than it holds), evidence must exist and belong to the material's project. Legacy cement routes stay live untouched.
